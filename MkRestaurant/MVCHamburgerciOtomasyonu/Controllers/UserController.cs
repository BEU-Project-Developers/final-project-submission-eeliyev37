	...
	public class UserController : Controller
	{
		...
		public async Task<IActionResult> Add(UserAddDto userAddDto)
		{
			...
			if (validation.IsValid)
			{
				...
				if (result.Succeeded)
				{
					_toastNotification.AddSuccessToastMessage(Messages.User.Add(userAddDto.Email), new ToastrOptions { Title = "Successful" });
			...
			if (user != null)
			{
				...
				if (ModelState.IsValid)
				{
					...
					if (validation.IsValid)
					{
						...
						if (result.Succeeded)
						{
							_toastNotification.AddSuccessToastMessage(Messages.User.Update(userUpdateDto.Email), new ToastrOptions { Title = "Successful" });
							return RedirectToAction("Index", "User");
						}
						else
						{
		...
		public async Task<IActionResult> Delete(Guid userId)
		{
			...
			{
				_toastNotification.AddSuccessToastMessage(Messages.User.Delete(result.email), new ToastrOptions { Title = "Successful" });
		...
		public async Task<IActionResult> Profile(UserProfileDto userProfileDto)
		{
			if (ModelState.IsValid && userProfileDto.CurrentPassword != null)
			{
				...
				if (result)
				{
					_toastNotification.AddSuccessToastMessage("Profile update process completed", new ToastrOptions { Title = "Successful" });
				...
				{
					var profile = await _userService.GetUserProfileAsync();
					_toastNotification.AddErrorToastMessage("Profile update process could not be completed, Missing Information Available", new ToastrOptions { Title = "Failed" });
					return View(profile);
				}
			}
			else
			{
				...
				var profile = await _userService.GetUserProfileAsync();
				_toastNotification.AddErrorToastMessage("Profile update process could not be completed, you must enter your current password.", new ToastrOptions { Title = "Failed" });
				return View(profile);
			}
		}
	}
